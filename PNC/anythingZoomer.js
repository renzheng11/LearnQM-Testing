	...
	$.anythingZoomer = function(el, options) {
		...
		base.zoomAt = function(x, y, sz, internal) {
			...
			if (o.edit) { 
					...
					var p = (Math.round((-2/(446-37)*y+1+36*2/(446-37))*100)/100).toString()+ "+/-10"
					// base.edit.html( p.sub()+n.sup() +s.sub());
					// base.edit.html(x);

					x_cordi = 100/260*x
					y_cordi = (-2/(446-37)*y+1+37*2/(446-37))*100

					x_cordi_real=x
					y_cordi_real=y
					// base.edit.html(Math.round(x));
...
//a=100/270
			...
			//	'+10^(-20)eV'
				...
				// } else {
				// 	base.edit.html(Math.round(0) + ', ' + Math.round(0));
				// }

				 }

			if ( (x < -ex) || (x > base.smallDim[0] + ex) || (y < -ey) || (y > base.smallDim[1] + ey) || (settings.nucleus==true)) {
	...
	$.anythingZoomer.defaultOptions = {
		...
		offsetX     : -100,          // adjust the horizontal position of the large content inside the zoom window as desired
		offsetY     : -20,          // adjust the vertical position of the large content inside the zoom window as desired
		// functionality
