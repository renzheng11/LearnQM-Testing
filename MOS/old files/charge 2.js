class Vehicle {
			...
			if (this.color == 0 && sceneCount != 3) {
				...
				if (this.origin.x == 0 && this.origin.y == 0) {
					...
					if (this.straight == 0) {
						...
						ellipse(
							this.position.x,
							this.origin.x - ((this.botz * 8.8 * 2 * 0.1) / 3) * s_y,
							5
						);
						this.position2.y =
							this.origin.x - ((this.botz * 8.8 * 2 * 0.1) / 3) * s_y;
				...
				if (this.origin.x == 0 && this.origin.y == 0) {
					...
					if (this.straight == 0) {
						...
						ellipse(
							this.position.x,
							this.origin.y +
								((this.botz * 8.8 * 2 * 0.1) / 3) * bandDiagramVScale * s_y,
							5
						);
						this.position2.y =
							this.origin.y + ((this.botz * 8.8 * 2 * 0.1) / 3) * s_y;
						this.position2.x = this.position.x;
					} else {
					}
				}
	...
	random_walk() {
		...
		if (this.position.y < 405 * s_y) {
			this.velocity.y = -this.velocity.y;
			this.position.y = 410 * s_y;
		}
		if (this.position.y > 770 * s_y) {
			this.velocity.y = -this.velocity.y;
			this.position.y = 765 * s_y;
		}
		if (this.position.x < 260 * s_x) {
			this.velocity.x = Math.abs(this.velocity.x);
			this.position.x = 263 * s_x;
		}

		// if ((this.position.x > 943*s_x)) {
		//     this.velocity.x = -Math.abs(this.velocity.x);
		//     this.position.y = random (410*s_y,770*s_y);
		...
		if (sceneCount == 2) {
			if (this.push == 0) {
				...
				if (this.position.x > 940 * s_x && opening == 1 && this.straight == 0) {
					...
					//     // initialHoles.push(new Vehicle(160*s_x, 600*s_y, 10, "h", 1));
				...
				// if (this.position.x < 260*s_x  ) {this.velocity.x = -this.velocity.x;}
			...
			if (this.position.x > 940 * s_x && opening == 0) {
				this.direction.x = -1;
			}
			if (this.position.y < (20 + 385) * s_y) {
				this.direction.y = 1;
			}
			if (this.position.y > 770 * s_y) {
				this.direction.y = -1;
	...
	opposite_walk() {
		...
		if (
			...
			this.position.x > 950 * s_x - this.diameter
		) {
			this.direction.x *= -1;
		}
		if (
			this.position.y < this.diameter ||
			this.position.y > 950 * s_y - this.diameter
		) {
			this.direction.y *= -1;
		}
	}
