...
function drawBands() {
	...
	let bandLength = 600; // original: 134
	...
	for (var k = 0; k < bandLength; k++) {
		...
		let x2 = 349;
		let y1 = 0;
		let y2 = 679; // changing this scale will horizontally stretch but position misplaced
		let a = (y2 - y1) / (x2 - x1);
		let b = y1 - a * x1;
		let xPos = a * dataArray1E14_xPos[k] + b;
		...
		// vertex drawn from currentArray
		curveVertex((250 + xPos) * sx, (171.25 + currentArray[k] * 40 - 100) * sy); // multiplying xPos also scales the band horizontally
		electronBand[k] = [
			(250 + xPos) * sx,
			(171.25 + currentArray[k] * (40 / 1.2) - 100) * sy,
		];
