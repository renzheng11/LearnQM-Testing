class Charge {
		...
		if (this.show == 1) {
			...
			if (this.color == 0 && sceneCount != 3) {
				...
				if (this.origin.x == 0 && this.origin.y == 0) {
					...
					ellipse(
						this.position.x,
						(s + 111 + this.botz * 8.8 * 0.2 * changeV) * s_y,
						5
					);
	...
	accelerate() {
		if (
			...
			if (this.color == 0) {
				let a =
					(e_field_c / (550 * s_x - (550 - (400 / 8) * count_pn_num) * s_x)) *
						(this.position.x - 550 * s_x) +
					((550 - (400 / 8) * count_pn_num) * s_x * e_field_c) /
...
class freeElectron {
	...
	random_walk_1() {
		if (this.scene == 1) {
			...
			if (this.position.x > 940 * s_x) {
				this.direction.x = -1;
			}
			if (this.position.y < 20 * s_y) {
				this.direction.y = 1;
			}
			if (this.position.y > 770 * s_y) {
				this.direction.y = -1;
